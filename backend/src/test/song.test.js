...
let token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY0ZTM4YjU1YTA1NmMxZTJlMzljNWU5NSIsImlhdCI6MTcwNDIwMzMxMSwiZXhwIjoxNzM1NzM5MzExfQ.zpQV7HAEsVR8qkS1v6RaunNjgRWYKF5JrQ_aThNhToc'
let creator
beforeAll(async () => {
...
describe("Songs Tests", () => {
    ...
    it("test getting all songs by ids", async () => {
        const response = await request(app).post("/songs/get-songs").send({ids: ["64e30d14fc68b9a5b37ba5bd", "64e30daffc68b9a5b37ba5c1"]});
        expect(response.statusCode).toEqual(200);
        expect(response.body).toBeDefined();
    })
    it("test getting all songs by ids that will fail", async () => {
        const response = await request(app).post("/songs/get-songs").send({ids: ["", ""]});
        expect(response.statusCode).toEqual(500);
    })
    ...
    it("create new song update it and delete it", async () => {
        const response = await request(app).post("/admin/songs/create").send({song: newSong, token: token});
        expect(response.statusCode).toEqual(201);
        expect(response.body).toBeDefined();
        updatedSong.creator = response.body.creator;


        const response2 = await request(app).put("/admin/songs/" + songId)
            .send({token: token, song: updatedSong, updatedSong: updatedSong});
        expect(response2.statusCode).toEqual(200);
        expect(response2.body).toBeDefined();

        updatedSong.duration = -1;
        const response4 = await request(app).put("/admin/songs/" + songId)
            .send({token: token, song: updatedSong, updatedSong: updatedSong});
        expect(response4.statusCode).toEqual(500);

        updatedSong.price = -1;
        const response5 = await request(app).put("/admin/songs/" + songId)
            .send({token: token, song: updatedSong, updatedSong: updatedSong});
        expect(response5.statusCode).toEqual(500);

        updatedSong.price = "a";
        const response6 = await request(app).put("/admin/songs/" + songId)
            .send({token: token, song: updatedSong, updatedSong: updatedSong});
        expect(response6.statusCode).toEqual(500);


        const response3 = await request(app).delete("/admin/songs/" + response.body._id)
    ...
    it("song update to fail", async () => {
        const response = await request(app).put("/admin/songs/" + songId)
    ...
    it("create new song to fail 2", async () => {
        newSong.price = -23;
        const response2 = await request(app).post("/admin/songs/create").send({song: newSong, token: token});
        expect(response2.statusCode).toEqual(500);
    })

    it("create new song to fail3 ", async () => {
        newSong.duration = -50000;
        const response3 = await request(app).post("/admin/songs/create").send({song: newSong, token: token});
